            ...
            with tf.variable_scope("scope_model"):
                ...
                for epoch in range(training_epochs):
                    ...
                    if epoch % display_step == 0:
                        #decoded_tmp = sess.run(output, feed_dict={x: testImages, y: testLabels, input_size: TEST_DATA_SIZE*CATEGORY})

                        cost_val = sess.run(cost, feed_dict={x: testImages, y: testLabels, input_size: TEST_DATA_SIZE*CATEGORY})
                        accuracy = sess.run(eval_op, feed_dict={x: testImages, y: testLabels, input_size: TEST_DATA_SIZE*CATEGORY})
                ...
                result_accuracy = sess.run(eval_op, feed_dict={x: testImages, y: testLabels, input_size: TEST_DATA_SIZE*CATEGORY})
                print('accuracy (test) = ', result_accuracy)

                decoded_imgs = sess.run(output, feed_dict={x: testImages, y: testLabels, input_size: TEST_DATA_SIZE*CATEGORY})
                decoded_imgs = decoded_imgs.reshape([-1, IMG_SIZE, IMG_SIZE, CATEGORY])
                if not os.path.exists('./result'):
                    os.mkdir('./result')
                for i in range(TEST_DATA_SIZE*CATEGORY):
                    for n in range(CATEGORY):
                        np.savetxt('./result/' + str(i) + '_' + str(n) + '.txt', decoded_imgs[i, :, :, n])
